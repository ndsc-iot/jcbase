...
<%@ page import="com.jcbase.model.SysUserRole"%>
...
//resize on sidebar collapse/expand
			...
			$("#btn_search").click(function(){  
			    ...
			    $("#grid-table").jqGrid('setGridParam',{  
			        datatype:'json',  
			        postData:{'name':name,
			        'username':'${sessionScope.sysUser.name}'}, //发送数据
			...
			$("#bnt-grant").click(function(){
				...
				if(rid == -1){
					...
					parent.layer.open({
					    ...
					    fix: false, //不固定
					    maxmin: true,
					    content: '${context_path}/sys/user/userRoleSetting?uid='+rid+'&uname=${sessionScope.sysUser.name}'
					});
				}
			});
      ...
      //replace icons with FontAwesome icons like above
		...
		function setVisible(status){
			...
			$.post("${context_path}/sys/user/setVisible", submitData,function(data) {
				if (data.code == 0) {
					layer.msg("操作成功", {
					    icon: 1,
